...
struct NwuivalNwiHome: View {
      ...
      VStack(alignment: .leading) {
        HStack {
          ZStack(alignment: .bottomLeading) {
            Rectangle().fill(VawinvTheme.FaioColor.gradientOrangeToYellow).frame(
              width: 80, height: 12
            ).cornerRadius(15)
            VStack {
          ...
          Circle().frame(width: 40, height: 40)
            .overlay(content: {
              ZwnagIreujImage(
                zwnagIreujImageUrl: userVM.currentUser?.feruyqCawdAvatar ?? "",
                zwnagIreujWidth: 40,
                zwnagIreujHeight: 40,
        ...
        HStack(spacing: 15) {
          Rectangle().fill(
            LinearGradient(
              colors: [
                Color(red: 141 / 255, green: 88 / 255, blue: 253 / 255),
          ...
          .onTapGesture {
            appPath.append(AppRoute.aiScriptWritingPresets)
          }
          Rectangle().fill(
            LinearGradient(
              colors: [
          ...
          .onTapGesture {
            appPath.append(AppRoute.aiCharacterChatPresets)
          }
        }.padding(.horizontal, 20)
        NwuicvalWorks(appPath: $appPath, currentType: $nwuivalCurrentType)
  ...
  struct NwuicvalWorks: View {
    ...
    var currentWorks: [WeianzVenvnWork] {
      currentType == .Trending
        ? workVM.allNotBlockWorks
        : workVM.myFollowingUserWorks
    }
    ...
    var body: some View {
      ...
      GeometryReader { geo in
        if currentWorks.isEmpty {
          XgkgaldEmptyDataPlaceholder(topPadding: 40)
