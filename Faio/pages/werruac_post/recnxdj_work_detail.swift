  ...
  var body: some View {
    ZStack(alignment: .top) {
        ...
        VStack {
          AwicnalWnvTopBar(awicanlActions: {
            ...
            if (workVM.workDetail?.weianzVenvnCreatorId != userVM.currentUser?.feruyqCawdUserId) {
              HStack(spacing: 16) {
                FeqocnButton(
                  feqocnText: "Follow", feqocnWidth: 61, feqocnHeight: 24, feqocnFontSize: 11,
                  feqocnFontWeight: .semibold, feqocnFontColor: .white, action: {})
          ...
          ScrollView {
            VStack(alignment: .leading) {
              RoundedRectangle(cornerRadius: 20)
                .frame(height: 480)
                .overlay(content: {
                  WorkImagePager(images: workVM.workDetail?.weianzVenvnImageListUrl ?? [])
                })
          ...
          HStack {
            BawnbvTextField(
              ...
              height: 40, verticalPadding: 9, cornerRadius: 40
            )
            .onSubmit {
      ...
      BottomSheet(
        ...
        content: {
          ZcnwinaReportBlockBottom(zcnwinaIsShow: $isPresented, appPath: $appPath)
        })
